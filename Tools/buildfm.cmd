...
:Usage
echo Usage: buildfm [oem/bsp/all] [bspname]
...
for %%B in ("%BLD_DIR%\buildfm_errors.txt") do if %%~zB gtr 0 (
    echo.   Error: Featuremerger failed for OEMFMFileList.xml. See %BLd_DIR%\buildfm_oem.log
)
exit /b 0
...
FeatureMerger %BLD_DIR%\InputFMs\%1FMFileList.xml %PKGBLD_DIR% %BSP_VERSION% %BLD_DIR%\MergedFMs /InputFMDir:%BLD_DIR%\InputFMs /Languages:en-us /Resolutions:1024x768 /ConvertToCBS /variables:_cputype=%BSP_ARCH%;buildtype=fre;releasetype=production > %BLD_DIR%\buildfm_bsp_%1.log

findstr /L "fatal" %BLD_DIR%\buildfm_bsp_%1.log > %BLD_DIR%\buildfm_errors.txt
for %%B in ("%BLD_DIR%\buildfm_errors.txt") do if %%~zB gtr 0 (
    echo.   Error: Featuremerger failed for %1FMFileList.xml. See %BLD_DIR%\buildfm_bsp_%1.log
)
exit /b 0
